...
ffmpeg -i "$file" -y -c:v libx264 -s 640x360 -b:v 800k -preset medium -movflags +faststart -pass 1 -an -f mp4 "lib/out.mp4" < /dev/null && ffmpeg -i "$file" -y -c:v libx264 -s 640x360 -b:v 800k -preset medium -movflags +faststart -pass 2 -c:a libfdk_aac -b:a 128k -metadata title="" -metadata artist="" "lib/$ffcomp" < /dev/null
mv "$file" "$ffbye"
#rm  "$ffile"
     else
